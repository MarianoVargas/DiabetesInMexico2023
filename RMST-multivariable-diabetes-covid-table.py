...
time_interval = 4  # Change this value to 4 for a 4-day interval

# Define the variables to consider (only DIABETES and NEUMONIA)
variables = ['DIABETES', 'TABAQUISMO' ]
...
while current_date <= end_date:
    ...
    if not interval_df.empty:
        ...
        for variable in variables:
            interval_df = interval_df[interval_df[variable] == 1]
...
plt.plot(dates, rmst_values, marker='o', linestyle='-', label='RMST', color='blue')
...
#
# # Define the variables to consider (only DIABETES and OBESIDAD)
# variables = ['DIABETES', 'OBESIDAD']
#
# # Create lists to store RMST estimates, 95% CI, and p-values
# rmst_estimates = []
# ci_lower_values = []
...
#         interval_rmst = np.mean(np.minimum(interval_df['TIEMPO_SUP'], time_horizon))
#
#         # Apply multivariable filter (only DIABETES and OBESIDAD)
#         for variable in variables:
#             if variable == 'DIABETES':
#                 interval_df = interval_df[interval_df[variable] == 1]
#             elif variable == 'OBESIDAD':
#                 interval_df = interval_df[interval_df[variable] == 1]
#
#         if not interval_df.empty:
#             interval_rmst_multivariable = np.mean(np.minimum(interval_df['TIEMPO_SUP'], time_horizon))
...
#     '95% CI': [(ci_lower, ci_upper) for ci_lower, ci_upper in zip(ci_lower_values, ci_upper_values)],
#     'p Value': [f'p < {p_value}' for p_value in p_values]
# })
#
